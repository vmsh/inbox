...
IMAPClient.prototype._responseRouter = function(data){
    ...
    if(data[0]=="*"){
        switch(data[1]){
            ...
            case "STATUS":
                var status = {};
                for (var i=0, len = data[3].length; i<len; i++) {
                    status[data[3][i].trim().toLowerCase()] = data[3][i+1];
                    i++;
                }
                this._status = status;
                return;
...
IMAPClient.prototype._formatEnvelope = function(envelopeData){
    ...
    // add size of message
    if (dataObject["RFC822.SIZE"]){
        message.size = dataObject["RFC822.SIZE"];
    }
...
IMAPClient.prototype._checkNewMail = function(){
    ...
    this._send("UID FETCH " + this._selectedMailbox.UIDNext + ":* (UID BODYSTRUCTURE FLAGS ENVELOPE INTERNALDATE RFC822.SIZE" +
      (this._capabilities.indexOf("X-GM-EXT-1") >= 0 ? " X-GM-LABELS X-GM-THRID" : "") +
      // Cyrus imap server 2.3.16 does not have MODSEQ in CONDSTORE, but have SORT=MODSEQ extention
      // (this._capabilities.indexOf("CONDSTORE") >= 0 ? " MODSEQ" : "") +
      (((this._capabilities.indexOf("CONDSTORE") >= 0) && (this._capabilities.indexOf("SORT=MODSEQ") == 0)) ? " MODSEQ" : "") +
      ")", (function(){
        this._checkForNewMail = false;
    }).bind(this),
    (function(){
...
IMAPClient.prototype.listMessages = function(from, limit, extendedOptions, callback){
    ...
    this._send(
      "FETCH " + from + ":" + to +
      " (UID BODYSTRUCTURE FLAGS ENVELOPE INTERNALDATE RFC822.HEADER RFC822.SIZE" + 
      // Cyrus imap server 2.3.16 does not have MODSEQ in CONDSTORE, but have SORT=MODSEQ extention
      // (this._capabilities.indexOf("CONDSTORE") >= 0 ? " MODSEQ" : "") +
      (((this._capabilities.indexOf("CONDSTORE") >= 0) && (this._capabilities.indexOf("SORT=MODSEQ") == 0)) ? " MODSEQ" : "") + 
      ")" +
 ...
 * Status
 */
IMAPClient.prototype.status= function(path,command,callback){
    console.log('path: ', path,',',command);
    this._rootMailbox.status(path,command, callback);
};
