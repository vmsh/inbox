...
/**
 * Fetches status of mailbox
 *
 * @param {String} name Name(path) mailbox
 * @param {String} args Aruments for imap STATUS command
 * @param {Function} callback Callback function to run with the mailbox
 */
Mailbox.prototype.status = function(name, args, callback){
    if(!callback && typeof args == "function"){
        callback = args;
        args = '(MESSAGES UNSEEN RECENT UIDNEXT UIDVALIDITY)';
    }
    args = args.toUpperCase(); //just in case :)

    var path = (this.path ? this.path + this.delimiter + name:name);
    this.client._send("status "+this.client._escapeString(path)+" "+args,
        (function(status){
            if (status == "OK") {
                callback(null, this.client._status);
            } else {
                callback(new Error("Getting status failed"));
            }
        }).bind(this));
};
